    ...
    for (var i = 0; i < 6; i++) {
      for (var j = 0; j < 3; j++) {
        for (var k = 0; k < 3; k++) {
		  ...
		  if(k==2 && j==2)
		  {
		  var face = stateWithCenters[drawingStickerMap[i][j-1][k-1]];
		  var LabelOrigin=scalePoint(w,h,[drawingCenters[i][0], drawingCenters[i][1]]);
		  attrib = {fill: "#000", stroke: "#000", font:"16px Helvetica", opacity:0.75};
